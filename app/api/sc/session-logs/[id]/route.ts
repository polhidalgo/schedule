...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  const { data: syncedRow } = await supabase
    .from('sc_session_logs')
    .select('timetable_session_id, attended, notes')
    .eq('id', id)
    .eq('user_id', user.id)
    .single()
  ...
  if (syncedRow) {
    const { error: syncError } = await syncScSessionLogToCalendar(supabase, user.id, {
      timetable_session_id: syncedRow.timetable_session_id ?? null,
      attended: syncedRow.attended ?? true,
      notes: syncedRow.notes ?? null,
    })
